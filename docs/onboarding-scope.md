# contract-ops-agent — OpenClaw-style Onboarding (v0.2.0 scope)
...
```
$ npx contract-ops-agent                 # first run
  ...
  → saved ~/.config/contract-ops-agent/config.json
  contract-ops-agent>                    # drops into the REPL

$ contract-ops-agent                     # every later run: straight to the REPL
$ contract-ops-agent setup               # re-run the wizard
$ contract-ops-agent doctor              # re-check environment, offer installs
```
...
| `bin/contract-ops-agent.mjs` | `setup` / `doctor` subcommands; first-run auto-runs setup; load config + `applyAuth` before the REPL |
...
1. Fresh machine (no config): `contract-ops-agent` runs the wizard, then the REPL; second launch skips to the REPL.
2. `contract-ops-agent doctor` reports CLI/backend/auth status and offers installs.
